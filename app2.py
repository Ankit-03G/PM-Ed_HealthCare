...
def save_data():
    data = {
        ...
        "injury_assessments": st.session_state.injury_assessments
    }
...
# Load existing data on app star
load_data()
...
# Analytics page
    ...
    if uploaded_file is not None and description and analyze_button:
        with st.spinner("Analyzing your injury..."):
            ...
            # Add to the list of injury assessments
    ...
    # Display previous injury assessments
    if st.session_state.injury_assessments:
        st.markdown("### Previous Injury Assessments")
        for assessment in st.session_state.injury_assessments:
            with st.expander(f"{assessment['patient_name']} - {assessment['timestamp']}"):
                st.markdown(f"**Patient Name:** {assessment['patient_name']}")
                st.markdown(f"**Patient Age:** {assessment['patient_age']}")
